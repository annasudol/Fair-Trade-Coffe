...
contract SupplyChain is FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  function packItem(uint _upc) public processed(upc) onlyFarmer()
  // Call modifier to check if upc has passed previous supply chain stage
  ...
  function sellItem(uint _upc, uint _price) public packed(upc) onlyFarmer
  // Call modifier to check if upc has passed previous supply chain stage
  ...
  // Use the above modifers to check if the item is sold @nextContract
  function shipItem(uint _upc) public onlyDistributor() sold(upc) 
    // Call modifier to check if upc has passed previous supply chain stage
